...
function renderImageRubricChart(vector, safe) {
  ...
  const labels = RUBRIC_KEYS.map((key, index) => {
    ...
    return `<text x="${pos.x.toFixed(1)}" y="${(pos.y + dy).toFixed(1)}" text-anchor="${anchor}" dominant-baseline="middle" fill="#3730a3" font-size="11" font-weight="800">${escape(IMAGE_RUBRIC_LABELS[key])}</text>`;
  }).join('');
