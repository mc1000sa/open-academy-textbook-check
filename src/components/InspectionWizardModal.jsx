...
export default function InspectionWizardModal({
  ...
  const executeSave = async (showPrompt = true) => {
    ...
    if (duplicate && showPrompt) {
      const ok = await showModalConfirm('같은 학생/교재/날짜/범위의 점검 기록이 이미 있습니다. 기존 기록을 덮어쓸까요?');
      if (!ok) return false;
    }
    ...
    const carryoverResolutions = buildCarryoverResolutions(currentCarryover, [...selectedCarryoverKeys]);
    const carryoverRecovery = calculateCarryoverRecoveryRate(currentCarryover, [...selectedCarryoverKeys]);
    ...
    const payload = {
      ...
      standardUnitIds,
      memo,
      rubricScores: normalizeRubricScores(rubricScores),
  ...
  // [순회 저장 및 다음] 핸들러
  // 현재 학생 정보 저장 ➔ 시작/끝 범위는 유지 ➔ 미완료 체크 리스트 등 양식만 초기화
  const handleSaveAndNext = async () => {
    ...
    if (nextStudent) {
      // 폼 리셋 (단, rangeStart / rangeEnd 시작/끝 범위는 그대로 복사 유지)
      ...
      const nextBook = assignedBooks[0]?.book || currentBook;
      ...
      setStep(1);
      showModalAlert(`${currentStudent.name} 점검 완료. 다음 학생(${nextStudent.name})으로 전환되었습니다.`, '반 순회 진행');
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in overflow-y-auto">
      ...
      <div className="relative w-full max-w-4xl bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto flex flex-col justify-between min-h-0 bg-slate-900">
          ...
          {/* 단계별 입력 패널 */}
          <main className="p-6 flex-1 overflow-y-auto flex flex-col justify-between">
            <div>
              {/* 단계 진행 안내 바 */}
              <div className="flex items-center gap-1.5 mb-6 overflow-x-auto pb-2 select-none mini-scroll">
                {[
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-[10px] font-black border transition-all cursor-pointer ${step === item.s ? 'bg-blue-600 border-blue-500 text-white shadow-sm' : 'bg-slate-950 border-slate-850 text-slate-500 hover:text-slate-300'}`}
                  >
                    {item.l}
                  </button>
                ))}
                    ...
                    <div className="space-y-4 mt-4">
                      {carryoverRows.map((row, i) => (
                        <div key={i} className="border border-slate-850 bg-slate-950/20 rounded-xl p-3">
                          <div className="text-[10px] font-black text-slate-500 mb-2">{fmtDate(row.sourceDate)} 점검 미완료 건</div>
                ...
                <div className="space-y-4 py-2">
                  ...
                  <div className="space-y-3.5">
                    {manualRubricItems.map(item => {
                      ...
                      return (
                        <div key={item.key} className="flex items-center gap-3 border border-slate-855 bg-slate-950/10 p-2.5 rounded-xl">
                ...
                {students.map(stud => {
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-xl px-4 py-2.5 border text-xs font-black transition-all cursor-pointer ${isCurrent ? 'bg-blue-600 border-blue-500 text-white shadow-md' : 'bg-slate-900 border-slate-800 text-slate-400 hover:border-slate-700 hover:text-white'}`}
                    >
                      {stud.name}
                    </button>
                  );
